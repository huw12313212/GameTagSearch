...
var URLPrefix = "http://www.gamestop.com";
var URLSearchPrefix = "/browse?nav=16k-3-";
var URLPostfix = ",28zu0";
var WebsiteName ="GameStop";
var options = {
    headers: {
...
GameStop.SearchTag = function(name,callback)
{
		options.url = URLPrefix+URLSearchPrefix+name+URLPostfix;
		...
		request(options, function (error, response, body) 
		{
		  ...
		  if (!error && response.statusCode == 200) 
		  {
		  	...
		  	//console.log(body);
		  	var listRow = $('.product_info h3 a');

		  	if(listRow.length>0)
		  	{
		  		var target = listRow[0];

		  		//console.log("game:"+$(target).text()+" url:"+$(target).attr("href"));

				options.url = URLPrefix+$(target).attr("href");
				//console.log("Try to Get:"+options.url);
				...
				request(options, function (error, response, body) 
				{
				  if (!error && response.statusCode == 200) 
				  {
				  		...
				  		//console.log(body);
				  		 ...
				  		 var listRow = $('.gameinfo ul li span');
				  		 var item = $(listRow[3]);

				  		 category = $(listRow[3]).text().replace("&",",").split(",");
